...
const DetailView = ({ agent, onClose, agentManager }) => {
  ...
  const isSearchMatch = (globalIndex) => searchMatches.includes(globalIndex);
  ...
  return React.createElement(Box, { flexDirection: 'column', height: '100%' }, [
    ...
    React.createElement(Box, {
      ...
      React.createElement(Box, { key: 'header-info', justifyContent: 'space-between' }, [
        React.createElement(
          Text,
          { key: 'name', bold: true, color: 'green' },
          `Agent Detail: ${agent.name}`,
        ),
        React.createElement(
          Text,
          { key: 'status', color: 'yellow' },
          `Status: ${agent.status}`,
        ),
      ]),
      searchQuery && React.createElement(
        Text,
        { key: 'search-info', color: 'cyan' },
    ...
    React.createElement(Box, {
      ...
      ...visibleLogs.map((log, index) => {
        ...
        return React.createElement(Box, { key: globalIndex }, [
          React.createElement(
            Text,
            { key: 'line-num', color: 'gray', dimColor: true },
            `${lineNumber} `,
          ),
      ...
      // Scroll indicators
      scrollOffset > 0 && React.createElement(
        Box,
        {
          key: 'scroll-up',
          position: 'absolute',
      ...
      scrollOffset + contentHeight < logs.length && React.createElement(
        Box,
        {
          key: 'scroll-down',
          position: 'absolute',
