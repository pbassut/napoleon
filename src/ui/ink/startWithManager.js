...
async function startFallbackUI(agentManager) {
    logger_js_1.default.info('Starting console interface (Ink not supported in this environment)');
...
async function startInkWithManager(agentManager) {
    ...
    if (!isInkSupported()) {
        logger_js_1.default.info('Ink UI not supported in current environment, using fallback');
        await startFallbackUI(agentManager);
        return;
    }
    try {
        // Create the App component
        const App = await (0, createApp_1.default)();
        // Create the app element with AgentManager
        const appElement = react_1.default.createElement(App, { agentManager });
        ...
        let result;
        try {
            result = (0, ink_1.render)(appElement, { debug: debugMode });
        }
        catch (renderError) {
            if (renderError.message.includes('Raw mode is not supported')) {
                logger_js_1.default.warn('Raw mode not supported, falling back to console UI');
                await startFallbackUI(agentManager);
                return;
            }
            throw renderError;
        ...
        await waitUntilExit();
        logger_js_1.default.info('Ink UI closed');
    }
    catch (error) {
        logger_js_1.default.error('Failed to start Ink UI with AgentManager', { error: error.message });
        // Try fallback UI if Ink fails
        if (error.message.includes('Raw mode is not supported')) {
            logger_js_1.default.info('Attempting fallback UI due to raw mode issue');
            await startFallbackUI(agentManager);
        }
        else {
            throw error;
...
exports.default = startInkWithManager;
