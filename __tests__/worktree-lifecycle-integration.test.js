...
describe('WorktreeLifecycleManager Integration', () => {
  ...
  describe('initialization', () => {
    it('should initialize successfully with valid worktree state', async () => {
      ...
      expect(result.metrics.discoveredWorktrees).toBe(0); // Discovery happens in background
      expect(result.metrics.recoveredWorktrees).toBe(0); // Recovery happens in background

      expect(mockDiscovery.validateWorktreeState).toHaveBeenCalled();
      // Note: Background discovery may start during initialize in test environment
